...
def compose_email(messages: list, role: str, kpis: dict,
    ...
    total_apps   = kpis.get("total_apps", 0)
    avg_rating   = kpis.get("avg_rating", 0)
    total_reviews= kpis.get("total_reviews", 0)
    pct_free     = kpis.get("pct_free", 0)
    top_cat      = kpis.get("top_category", "N/A")

    # ── KPI rows ───────────────────────────────────────────────────────────────
    kpi_rows = f"""
      <tr>
        <td style="padding:8px 14px;color:#64748b;font-size:14px">Total Apps</td>
        <td style="padding:8px 14px;text-align:right;font-weight:600;color:#2563eb;font-size:14px">{total_apps:,}</td>
      </tr>
      <tr style="background:#f8fafc">
        <td style="padding:8px 14px;color:#64748b;font-size:14px">Avg Rating</td>
        <td style="padding:8px 14px;text-align:right;font-weight:600;color:#16a34a;font-size:14px">{avg_rating:.2f} / 5.0</td>
      </tr>
      <tr>
        <td style="padding:8px 14px;color:#64748b;font-size:14px">Total Reviews</td>
        <td style="padding:8px 14px;text-align:right;font-weight:600;color:#d97706;font-size:14px">{total_reviews:,}</td>
      </tr>
      <tr style="background:#f8fafc">
        <td style="padding:8px 14px;color:#64748b;font-size:14px">Free / Freemium %</td>
        <td style="padding:8px 14px;text-align:right;font-weight:600;color:#2563eb;font-size:14px">{pct_free:.1f}%</td>
      </tr>
      <tr>
        <td style="padding:8px 14px;color:#64748b;font-size:14px">Largest Category</td>
        <td style="padding:8px 14px;text-align:right;font-weight:600;color:#7c3aed;font-size:14px">{top_cat}</td>
      </tr>
    """
    ...
    subject = f"Business AI Copilot — {role} App Store Analysis — {date_str}"
        ...
        <tr>
          <td style="background:#f8fafc;border-top:1px solid #e2e8f0;
            ...
            <p style="margin:0;font-size:12px;color:#94a3b8;line-height:1.6">
              ...
              Shopify App Store Dataset &nbsp;·&nbsp; This is an automated summary.
            </p>
          </td>
        </tr>
